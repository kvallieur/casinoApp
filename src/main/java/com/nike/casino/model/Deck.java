...
public class Deck<T> {
	...
	private List<T> handShuffle(List<T> deck) {
		List<T> halfDeck1 = new LinkedList<>();
		List<T> halfDeck2 = new LinkedList<>();
		List<T> shuffledDeck = new LinkedList<>();
		for (int i = 0; i < deck.size(); i++) {
			if (i < (deck.size() / 2)) {
				halfDeck1.add(deck.get(i));
			} else {
				halfDeck2.add(deck.get(i));
			}
		}
		// assuming there are two equal halves
		if (halfDeck1.size() == halfDeck2.size()) {
			for (int j = 0; j < halfDeck1.size(); j++) {
				shuffledDeck.add(halfDeck1.get(j));
				shuffledDeck.add(halfDeck2.get(j));
			}
		}
	...
	private List<T> quarterShuffle(List<T> deck) {
		...
		List<T> quarterDeck1 = new LinkedList<>();
		List<T> quarterDeck2 = new LinkedList<>();
		List<T> quarterDeck3 = new LinkedList<>();
		List<T> quarterDeck4 = new LinkedList<>();

		// assuming that deck can be divided into quarters
		if ((deck.size() % 4) == 0) {
			int quarterNum = deck.size() / 4;
			for (int i = 0; i < quarterNum; i++) {
				quarterDeck1.add(deck.get(i));
				quarterDeck2.add(deck.get(i + quarterNum));
				quarterDeck3.add(deck.get(i + (2 * quarterNum)));
				quarterDeck4.add(deck.get(i + (3 * quarterNum)));
			}
			for (int k = 0; k < quarterNum; k++) {
				shuffledDeck.add(quarterDeck1.get(k));
				shuffledDeck.add(quarterDeck2.get(k));
				shuffledDeck.add(quarterDeck3.get(k));
				shuffledDeck.add(quarterDeck4.get(k));
			}
		}
